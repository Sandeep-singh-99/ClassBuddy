...
//       setSubmitting(true);
//        const payload = {
//       answers, // your {questionId: "answer"} object
//     };

//       dispatch(SubmitSubmission({ id: assignmentId! , data: payload }))
//     } catch (err) {
//       console.error(err);
//       setSubmitting(false);
//     }
    ...
    >
      ...
      <Card className="p-6 rounded-xl bg-zinc-900 border border-zinc-800 shadow-md">
        <CardContent>
          ...
          <p className="mt-2 text-gray-400">{currentAssignment.description}</p>
          <p className="text-sm mt-3 text-gray-500">
            Due:{" "}
            <span className="font-medium text-gray-300">
              {new Date(currentAssignment.due_date).toLocaleString()}
        ...
        <Card className="bg-zinc-900 border border-zinc-800 space-y-5">
          {questions.map((q: any, index: number) => {
            ...
            const gradedAnswer = submitted
              ? submissionState.result?.answers?.find((a: any) => a.id === q.id)
                  ?.answer
              : null;
