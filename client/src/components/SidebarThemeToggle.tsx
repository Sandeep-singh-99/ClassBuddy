...
export function SidebarThemeToggle() {
  ...
  return (
    <SidebarMenuButton
      ...
      className={`relative overflow-hidden group hover:bg-muted/50 transition-all duration-300 border border-transparent hover:border-border/40 hover:shadow-sm ${open ? '' : 'justify-center'}`}
    >
      <div className={`relative flex items-center justify-center w-5 h-5 ${open ? 'mr-1' : ''}`}>
        <Sun
