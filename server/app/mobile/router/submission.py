...
async def assignment_stats(
    ...
    if not group:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Group not found for this assignment",
        )
    ...
    students_completed = (
        db.query(Submission).filter(Submission.assignment_id == assignment_id).count()
    )
...
async def student_submissions_stats(
...
def get_student_assignments(
    ...
    if current_user.role != userRole.STUDENT:
        raise HTTPException(
            status_code=403, detail="Only students can access this route"
        )
        ...
        for sub in submissions:
            ...
            if assignment:
                grades_vs_assignments.append(
                    {"assignment_title": assignment.title, "grade": sub.grade or 0}
                )
