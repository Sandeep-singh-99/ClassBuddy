...
def get_teacher_notes_with_docs(
    db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    if not current_user:
        raise HTTPException(
    ...
    if len(docs.groups) == 0:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No groups found for the user"
        )

    group_ids = [group.id for group in docs.groups]

    docs_with_notes = (
        db.query(DocsUpload).filter(DocsUpload.group_id.in_(group_ids)).all()
    )
    docs_with_notes.sort(key=lambda x: x.updated_at, reverse=True)

    return {"count": len(docs_with_notes), "docsuploads": docs_with_notes}
...
def delete_doc(
    ...
    doc = (
        db.query(DocsUpload)
        .filter(DocsUpload.id == doc_id, DocsUpload.owner_id == current_user.id)
        .first()
    )
    if not doc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="You not authorized to delete this document",
        )
