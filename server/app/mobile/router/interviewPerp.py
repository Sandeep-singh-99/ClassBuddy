...
@router.post(
    "/create-interview-prep", response_model=InterviewPreparationCreateResponse
)
...
async def submit_quiz(
    submission: InterviewPreparationResponse,
    ...
    query = (
        ...
        .filter(
            ...
            InterviewPrep.name == submission.name,
        )
        .first()
    )
    if query:
        raise HTTPException(
            status_code=400, detail="Quiz with this name already submitted."
        )
