...
from app.api.v1.endpoints import auth, ai_evaluator, assignment, generate_assignment, generate_notes, group, groupMessage, teacherInsight, studentInsight, submission, subscription, inngest_route, interviewPerp, docsupload, health

app_router = APIRouter()

app_router.include_router(auth.router, prefix='/auth', tags=['Authentication'])
app_router.include_router(ai_evaluator.router, prefix='/ai-evaluator', tags=['AI Evaluator'])
app_router.include_router(assignment.router, prefix='/assignments', tags=['Assignments'])
app_router.include_router(generate_assignment.router, prefix='/assignments', tags=['Generate Assignments'])
...
app_router.include_router(teacherInsight.router, prefix='/insights', tags=['Techer Insights'])
app_router.include_router(studentInsight.router, prefix='/student-insight', tags=['Student Insights'])
app_router.include_router(submission.router, prefix='/submissions', tags=['Submissions'])
