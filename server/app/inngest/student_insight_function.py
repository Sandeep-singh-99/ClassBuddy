...
#             "getIndustry": [],
#         }
#         # Ensure your graph is called with ainvoke for async compatibility
...
#                 .filter(StudentInsight.user_id == user_id, StudentInsight.industry == industry)
#                 .first()
#             )
#             if insight:
...
#             else:
#                 insight = StudentInsight(user_id=user_id, industry=industry, **insight_data)
#                 db.add(insight)
#             db.commit()
#             return True
...
#     await step.run("clear-redis-cache", lambda: get_redis_client().delete(f"student_insights:{user_id}"))
